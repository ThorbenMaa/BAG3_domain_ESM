...
"""
command python esm_calc_matshow_instead_of_imshow.py
"""
import torch
...
@click.option(
    ...
    required=True,
    type=str,
    default="data/output/output_heatmap_colormesh.svg",
    help="Output file",
)
def cli(input_file, modelsToUse, output_heatmap):
    ...
    for k in range(0, len(modelsToUse), 1):
        ...
        # plot
        plt.figure(figsize=(token_probs_np.shape[0]/5, 8))
        y_list=["L","A","G","V","S","E","R","T","I","D","P","K","Q","N","F","Y","M","H","W","C"]
            
        plt.pcolormesh(np.transpose(token_probs_np) )

        plt.yticks(np.arange(len(y_list)), labels=y_list, fontsize=10) #np.arange(len(y_list)),
